...
import { Search, Filter, Package as PkgIcon, AlertTriangle, TrendingUp, Sparkles } from "lucide-react";
...
export const Route = createFileRoute("/_app/products" as never)({
  head: () => ({
    meta: [
      { title: "Product Intelligence — OmniMind AI" },
      { name: "description", content: "Product 360° intelligence with sales, demand, expiry, and reorder recommendations." },
    ],
  }),
  component: Products,
});
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <MiniKpi label="Total SKUs" value="8,412" icon={<PkgIcon className="h-4 w-4" />} />
        <MiniKpi label="Active Products" value="7,884" tone="success" />
        <MiniKpi label="Low Stock" value="128" tone="warning" icon={<AlertTriangle className="h-4 w-4" />} />
        <MiniKpi label="Best-selling Dept" value="Fashion" icon={<TrendingUp className="h-4 w-4" />} />
      </div>
          ...
          actions={
            <div className="relative">
              ...
              <Input
                ...
                className="h-8 w-64 bg-background pl-8 text-xs"
              />
            </div>
          }
        >
                  ...
                  >
                    <td className="py-2.5">
                      <div className="flex items-center gap-2.5">
                        <div className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-surface-2 text-[10px] font-semibold text-muted-foreground">
                    ...
                    <td className="py-2.5 text-right">
                      <span className={cn(p.stock < p.reorder / 4 ? "text-destructive" : p.stock < p.reorder ? "text-warning" : "")}>
                        {fmtNum(p.stock)}
                      </span>
                    </td>
  ...
  return (
    <div className="card-elevated p-3.5">
      <div className="flex items-center justify-between text-[10px] uppercase tracking-wider text-muted-foreground">
        {label}
        {icon && <span className={tone === "warning" ? "text-warning" : "text-primary"}>{icon}</span>}
      </div>
      <p className={cn("mt-2 font-display text-xl font-semibold", tone === "success" && "text-success")}>{value}</p>
    </div>
  );
}

function Product360({ p }: { p: (typeof PRODUCTS)[number] }) {
      ...
      <SectionCard>
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{p.dept} · {p.category}</p>
            <h3 className="mt-0.5 truncate font-display text-lg font-semibold">{p.name}</h3>
            <p className="text-xs text-muted-foreground">{p.id} · {p.brand} · Supplier: {p.supplier}</p>
          </div>
        </div>
        ...
        {p.expiry && (
          <div className="mt-3 rounded-md border border-warning/40 bg-warning/10 p-3 text-xs">
            ...
            <p className="mt-1 text-foreground/90">
              Batch B-{p.id.slice(-4)} expires{" "}
              <span className="font-semibold">{p.expiry}</span> · {fmtNum(Math.round(p.stock * 0.4))} units affected
            </p>
          </div>
        )}
        ...
        <div className="mt-3 rounded-md border border-primary/30 bg-primary/5 p-3 text-xs">
          ...
          <p className="mt-1 text-foreground/90">
            At current demand, stock will finish in <span className="font-semibold">{daysLeft} days</span>.
            Recommended reorder: <span className="font-semibold">{Math.round(p.sold / 30 * 14)} units</span> to
            cover a 14-day buffer.
          </p>
