...
import { CUSTOMERS, fmtINR, fmtNum } from "@/lib/mock-data";
import { Bar, BarChart, CartesianGrid, Cell, Legend, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import { useState } from "react";
import { cn } from "@/lib/utils";

export const Route = createFileRoute("/_app/customers" as never)({
  head: () => ({
    meta: [
      { title: "Customer Intelligence — OmniMind AI" },
      { name: "description", content: "Segments, lifetime value, churn risk, and customer 360° profiles." },
    ],
  }),
  component: Customers,
});
...
function Customers() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Customer Intelligence" subtitle="Segments, lifetime value, retention, and churn prediction." />

      <div className="grid grid-cols-2 gap-3 md:grid-cols-6">
        <Kpi label="Total Customers" v="14,248" />
        ...
        <SectionCard title="Segments" className="xl:col-span-1">
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={SEGMENTS} dataKey="v" nameKey="name" innerRadius={44} outerRadius={80} paddingAngle={2} stroke="var(--color-background)" strokeWidth={2}>
                  {SEGMENTS.map((_, i) => (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={Array.from({ length: 30 }, (_, i) => ({
                  d: `${i + 1}`,
                  new: Math.round(20 + Math.sin(i / 2) * 12 + Math.random() * 12),
                  returning: Math.round(60 + Math.cos(i / 3) * 20 + Math.random() * 20),
                }))}
              >
                ...
                <Bar dataKey="returning" fill="var(--color-cyan)" name="Returning" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </SectionCard>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.4fr_1fr]">
              ...
              <tbody className="divide-y divide-hairline">
                {CUSTOMERS.map((c) => (
                  <tr
                    key={c.id}
                    onClick={() => setSel(c)}
                    className={cn("cursor-pointer hover:bg-surface-2/40", sel.id === c.id && "bg-primary/8")}
                  >
                    <td className="py-2.5">
                      <p className="font-medium">{c.name}</p>
                      <p className="text-[10px] text-muted-foreground">{c.id} · joined {c.joined}</p>
                    </td>
                    <td className="py-2.5 text-right">{fmtNum(c.visits)}</td>
                    <td className="py-2.5 text-right font-semibold">{fmtINR(c.spend, { compact: true })}</td>
                    <td className="py-2.5 text-right">{fmtINR(c.aov)}</td>
                    <td className="py-2.5 text-muted-foreground">{c.favDept}</td>
                    <td className="py-2.5">
                      <StatusPill
                        tone={
                          c.segment === "VIP" ? "violet" :
                          c.segment === "Loyal" ? "success" :
                          c.segment === "At Risk" ? "warning" :
                          c.segment === "Dormant" ? "danger" : "info"
                        }
                      >
                        {c.segment}
                      </StatusPill>
                    </td>
                    <td className={cn("py-2.5 text-right", c.churn > 60 ? "text-destructive" : c.churn > 25 ? "text-warning" : "text-success")}>
                      {c.churn}%
                    </td>
                  </tr>
                ))}
        ...
        <SectionCard title="Customer 360°" subtitle={sel.name}>
          <div className="grid grid-cols-2 gap-2">
            ...
            <Stat label="Churn Risk" v={`${sel.churn}%`} tone={sel.churn > 60 ? "danger" : "default"} />
          </div>

          <div className="mt-4">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Favorite Department</p>
...
function Stat({ label, v, tone }: { label: string; v: string; tone?: "default" | "danger" }) {
  return (
    <div className="rounded-md border border-hairline bg-surface p-2">
      <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</p>
      <p className={`mt-0.5 text-sm font-semibold ${tone === "danger" ? "text-destructive" : ""}`}>{v}</p>
    </div>
  );
}
